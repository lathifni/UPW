            ...
            <form action="{{ route('programs.update', $program->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select name="category" id="category"
                                ...
                                @foreach ($categories as $cat)
                                    <option value="{{ $cat }}"
                                        {{ old('category', $program->category) == $cat ? 'selected' : '' }}>
                                        {{ $cat }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="rekening_id" id="rekening_id"
                                ...
                                @foreach ($rekenings as $rek)
                                    <option value="{{ $rek->id }}"
                                        {{ old('rekening_id', $program->rekening_id) == $rek->id ? 'selected' : '' }}>
                ...
                <div class="form-group mb-3">
                    ...
                    <select class="form-control" id="certificate_type" name="certificate_type">
                        <option value="none"
                            {{ old('certificate_type', $program->certificate_type) == 'none' ? 'selected' : '' }}>Tidak
                            Ada (Default)</option>
                        <option value="sertifikat_standar"
                            {{ old('certificate_type', $program->certificate_type) == 'sertifikat_standar' ? 'selected' : '' }}>
                            Sertifikat Standar (Dinamis)</option>
                        <option value="akta_wakaf"
                            {{ old('certificate_type', $program->certificate_type) == 'akta_wakaf' ? 'selected' : '' }}>
                            Akta Ikrar Wakaf (Fixed)</option>
                        <option value="surat_apresiasi"
                            {{ old('certificate_type', $program->certificate_type) == 'surat_apresiasi' ? 'selected' : '' }}>
                            Surat Apresiasi Rektor (Fixed)</option>
                    </select>
                </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('deadline') is-invalid @enderror"
                                id="deadline" name="deadline" value="{{ old('deadline', $program->deadline) }}">
                            @error('deadline')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="col-md-4 mb-2">
                            ...
                            @if ($program->image)
                                <img src="{{ asset('storage/programs/' . $program->image) }}"
                                    class="img-fluid rounded border" style="max-height: 150px;" alt="Current Image">
                            @else
